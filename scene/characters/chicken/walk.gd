...
func character_setup() -> void:
	##等待一个物理帧
	##至于为什么要用call_deferred 还要使用 await 总之就是确保场景树初始化完成
	##把set_movement 和这个函数分开 因为这个函数第一次调用 后面都是使用set那个函数
	await get_tree().physics_frame
	##这个是为了让三个鸡不是同时出发 防止第一个目标终点一致
...
func _on_physics_process(_delta : float) -> void:
	...
	var v : Vector2 =target_direction * speed
	if navigation_agent_2d.avoidance_enabled:
		animated_sprite_2d.flip_h = v.x<0
		navigation_agent_2d.velocity = v

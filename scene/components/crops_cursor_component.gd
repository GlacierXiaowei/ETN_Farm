...
var corn_plant_pre = preload("res://scene/objects/plant/corn.tscn")
var tomato_plant_pre = preload("res://scene/objects/plant/tomato.tscn")
...
func get_cell_under_mouse() ->void:
	...
	var cell_global_position = dirt_tilemap_layer.to_global(local_cell_position)
...
func get_cell_in_front_of_player() -> void:
	...
	var target_local_position: Vector2 = dirt_tilemap_layer.to_local(target_global_position)
	
	# 4️⃣ 转成格子坐标
	cell_position = dirt_tilemap_layer.local_to_map(target_local_position)
	
	# 5️⃣ 读取这个格子的 source id
	cell_source_id = dirt_tilemap_layer.get_cell_source_id(cell_position)
	
	# 6️⃣ 计算距离（给你的统一逻辑用）
	var cell_local_position = dirt_tilemap_layer.map_to_local(cell_position)
	var cell_global_position = dirt_tilemap_layer.to_global(cell_local_position)
	distance = player.global_position.distance_to(cell_global_position)


func add_crop() -> void:
	##20.0 是指的像素
	##这个函数单纯变成添加耕地 判断不再由他负责(前面已经进行判断了)
	if distance< 25.0 :
		if ToolManager.selected_tool == DataType.Tools.PlantCorn:
			var corn_plant = corn_plant_pre.instantiate() as Node2D
			corn_plant.global_position = local_cell_position
			##这里会找父节点 请不要变换该节点的根节点
			get_parent().find_child("CropField").add_child(corn_plant)
		if ToolManager.selected_tool == DataType.Tools.PlantTomato:
			var tomato_plant = tomato_plant_pre.instantiate() as Node2D
			tomato_plant.global_position = local_cell_position
			##这里会找父节点 请不要变换该节点的根节点
			get_parent().find_child("CropField").add_child(tomato_plant)

func remove_grop() -> void:
	if distance< 25.0 :
		var crop_node = get_parent().find_child("CropField").get_children()
		
		for node: Node2D in crop_node:
...
func update_preview() -> void:
	if ToolManager.current_tool != DataType.Tools.PlantTomato:
		return

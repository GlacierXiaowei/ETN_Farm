...
var main_scene_level_root_path = "/root/MainScene/GameRoot/LevelRoot"
...
func load_level(level : String) -> void:
	...
	var level_scene: Node2D = load(scene_path).instantiate() as Node2D
	var level_root : Node2D = get_node(main_scene_level_root_path) as Node2D
	
	if level_root != null:
		##这种 会返回 level_root的子节点的array
